...
LANGS = {
    ...
    "zh-Hans": dict(out="zh",      path="/zh/",      htmllang="zh-Hans", locale="zh_CN", font="Noto+Sans+SC", name="简体中文"),
    "zh-Hant": dict(out="zh-hant", path="/zh-hant/", htmllang="zh-Hant", locale="zh_TW", font="Noto+Sans+TC", name="繁體中文"),
}
HREFLANG = {"ja": "ja", "en": "en", "zh-Hans": "zh-Hans", "zh-Hant": "zh-Hant"}

BASE_FONTS = ["Manrope:wght@400;500;600;700;800"]
# 原运行时是「值里含 < 就当 HTML，否则当纯文本」，这里照搬同一判定
ESCAPE_VALUES = "auto"
BODY_CLASS = None
STORAGE_KEY = "pc_lang"
BAR_KEY = {"en": "en", "zh-Hans": "zh", "zh-Hant": "zh-Hant"}
DETECT = {"en": "en", "zh-Hans": "zh-Hans", "zh-Hant": "zh-Hant"}
DEFAULT_LANG = "ja"
EXTRA_URLS = ["/privacy.html", "/terms.html"]
# 日文页用日文 UI 的截图，其余用英文 UI 那套
SHOTDIR = {"ja": "/shots/ja/"}
SHOTDIR_DEFAULT = "/shots/"

# 这个站的元素默认 opacity:0，靠这个观察器加 .in 才显示 —— 删掉页面就是空白的
EXTRA_JS = """(function(){
...
def fonts(lang):
    ...
    f = LANGS[lang]["font"]
    if f:
        fam.append("%s:wght@400;500;700" % f)
    if not fam:
...
def main():
    ...
    for lang, cfg in LANGS.items():
        ...
        vals = {
            ...
            "SHOTDIR": SHOTDIR.get(lang, SHOTDIR_DEFAULT),
        }
        for l2, c2 in LANGS.items():
